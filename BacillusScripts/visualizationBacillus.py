...
def visualizeBacillusFermentation(dict_for_graphs):
 ...
 for exp in dfg:
     ...
     fig.add_trace(go.Scatter(x=dfg[exp]["on"].index, y=dfg[exp]["on"][("SUBS_A2","Value")], mode="markers", name = "FeedPumpPower", marker= dict(color= col_dict["F_out"], size= 3, symbol="0"), legendgroup ="group7"), row=3, col=2,secondary_y=False)
