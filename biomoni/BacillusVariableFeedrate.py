...
class Bacillus_vf(Model):     #Dependent on base class
    ...
    def create_controls(self, experiment):
        ...
        , "cNH3" : experiment.metadata.loc["cNH3"], "gas_flow" : experiment.metadata.loc["gas_flow"], "T" : experiment.metadata.loc["T"]}

        c["pressure"] = self.calc_mean_pressure(experiment)

        feedpump_power = pd.to_numeric(experiment.dataset_raw["on"]["SUBS_A2"] , downcast="float" , errors="coerce") #in some experiments the feed_pump from MFCS was recognized as string instead of float...
    ...
    def create_y0(self, experiment):
        ...
        y0 = list(experiment.metadata.loc[["mS0", "V0"]].values)
