...
def visualizeBacillusFermentation(sim_data, Fdata= None, title= None,  suffix_1 = "", suffix_2 = "_fitted", mode_1 = "markers", mode_2 = "lines"):
    """X"""
    ...
    fig = make_subplots(rows= 4, cols=2, shared_xaxes ="all", start_cell = "top-left",specs =[[{"secondary_y" : True}, {"secondary_y" : True}],
                                                                                               [{"secondary_y" : True}, {"secondary_y" : True}],
                                                                                               [{"secondary_y" : True}, {"secondary_y" : True}],
                                                                                               [{"secondary_y" : True},{"secondary_y" : True}]], x_title = "F-time [h]")
    ...
    for i, data_list in data_dict.items():
        if data_list is None:
            ...
            for df in data_list:
                secondary_y_flag = col in sec_cols
                for col in df:
                     fig.add_trace(
                            go.Scatter(x= df.index, y= df[col], name = col+ suffix[i], mode = mode[i], 
                                       marker = dict(color = col_dict[col], size = 5, symbol = "x")), 
                            secondary_y= secondary_y_flag)
                     



    cols_y1 = [col for col in col_dict.keys() if col not in sec_cols]            
    cols_y2 = [col for col in col_dict.keys() if col in sec_cols]
